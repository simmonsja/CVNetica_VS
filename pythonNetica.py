...
class pynetica(object):
    ...
    ########################################
    # CVNetica_VS functions (alphabetical) #
    ########################################
        
    def checkNodes(self,netName,nodeNames):
        ### checks that all nodes named in nodeNames are present in
        ### the net at location netName
        cnet = self.pyt.OpenNeticaNet(netName)
        for nodeName in nodeNames:
            cnodnum = self.pyt.GetNodeNamed(ct.c_char_p(nodeName.encode()),cnet)
            if not cnodnum:
                # Done with Netica so shut it down
                self.pyt.DeleteNet(cnet)
                self.pyt.CloseNetica()
                raise NameError('Node "%s" is not present in %s.' % (nodeName, netName))
        self.pyt.DeleteNet(cnet)

    def cleanCVOutput(self):
        # reset the netica test data
        self.NeticaTests['CAL'] = list()
        self.NeticaTests['VAL'] = list()
    ...
    def deleteExtraNodes(self,netName,netOut,keepNodes):
        ...
        #store the net
        outfile_streamer = self.pyt.NewFileStreamer(ct.c_char_p(netOut.encode()))
        self.pyt.CompileNet(cnet) 
        vprint(3,self.pyt.verboselvl,"Writing new net with correct nodes to: {0:s}".format(netOut))
    ...
    def linkNodes(self,netName,netOut,parentNode,childNode):
        ...
        outfile_streamer = self.pyt.NewFileStreamer(ct.c_char_p(netOut.encode()))
        self.pyt.CompileNet(cnet) 
        vprint(3,self.pyt.verboselvl,"Writing new net with new node links to: {0:s}".format(netOut))
        self.pyt.WriteNet(cnet, outfile_streamer)
        self.pyt.DeleteStream(outfile_streamer)
        self.pyt.DeleteNet(cnet)

    ########################################
    # CVNetica functions (alphabetical)    #
    ########################################
        ...
        for ccas in np.arange(self.N):
            ...
            for cr in respnodes:
                ...
                for cp in parent_indices[cr].parent_names:
                    cnode = self.pyt.GetNodeNamed(ct.c_char_p(cp.encode()),cnet)
                    tmpinds.append(self.pyt.GetNodeFinding(cnode))
                parent_indices[cr].parent_indices.append(tmpinds)
        for cr in respnodes:
            vprint(3,self.pyt.verboselvl,'making into an array --> %s' %(cr))
    ...
    def PredictBayesNeticaCV(self, cfold, cnetname, calval):
        ...
        currcases = self.pyt.NewCaseset(ct.c_char_p('cval{0:d}'.format(np.abs(cfold)).encode()))
        ccaseStreamer = self.pyt.NewFileStreamer(ct.c_char_p(ccasefile.encode()))
        ...
        for cn in self.probpars.scenario.response:
            ...
            vprint(3,self.pyt.verboselvl,"LogLoss for {0:s} --> {1:f}".format(cn, ctestresults.logloss[cn]))
            # get error rate
            ctestresults.errrate[cn] = self.pyt.GetTestErrorRate(ctester, cnode)
            vprint(3,self.pyt.verboselvl,"ErrorRate for {0:s} --> {1:f}".format(cn, ctestresults.errrate[cn]))
            # get quadratic loss
            ctestresults.quadloss[cn] = self.pyt.GetTestQuadraticLoss(ctester, cnode)
            vprint(3,self.pyt.verboselvl,"QuadLoss for {0:s} --> {1:f}".format(cn, ctestresults.quadloss[cn]))
            # write confusion matrix --- only for the base case
            if cfold < 0:
                vprint(3,self.pyt.verboselvl,"Calculating confusion matrix for {0:s}".format(cn))
                ctestresults.confusion_matrix[cn] = self.pyt.ConfusionMatrix(ctester, cnode)
                # also calculate the experience for the node
                vprint(3,self.pyt.verboselvl,"Calculating Experience for the base Net, node --> {0:s}".format(cn))
    ...
    def ExperiencePostProc(self):
        vprint(3,self.pyt.verboselvl,'Post-Processing Experience data, matching with predicted nodes and cases')
        for cn in self.probpars.scenario.response:
            for ccas in np.arange(self.N):
    ...
    def predictBayes(self, netName, N, casdata):
        ...
        cpred = dict()
        vprint(3,self.pyt.verboselvl,'Making predictions for net named --> {0:s}'.format(netName))
        cnet = self.pyt.OpenNeticaNet(netName)
        #retract all the findings
        self.pyt.RetractNetFindings(cnet)
        for CN in cNETNODES:
        ...
        for i in np.arange(N):
            ...
            for cn in np.arange(numnodes):
                ...
                if cnodename in self.probpars.scenario.nodesIn:
                    self.pyt.EnterNodeValue(cnode, ct.c_double(casdata[cnodename][i]))
            for cn in np.arange(numnodes):
        ...
        for i in self.probpars.scenario.response:
            vprint(3,self.pyt.verboselvl,'postprocessing output node --> {0:s}'.format(i))
    ...
    def PredictBayesPostProc(self, cpred, outname, casname, cNeticaTestStats):
        ...
        ofp.write('%14s '*12
                  ...
                  + '\n')
        for i in self.probpars.scenario.response:
            vprint(3,self.pyt.verboselvl,'writing output for --> {0:s}'.format(i))
    ...
    def PredictBayesPostProcCV(self, cpred, numfolds, ofp, calval, cNeticaTestStats):
        for cfold in np.arange(numfolds):
            for j in self.probpars.scenario.response:
                vprint(3,self.pyt.verboselvl,'writing %s cross-validation output for --> %s' %(calval, j))
    ...
    def SummarizePostProcCV(self):
        ...
        for infile in [self.probpars.Cal_outfile, self.probpars.Val_outfile]:
            ...
            indat = np.genfromtxt(infile, skip_header=3, dtype=None, names=True)
    ...
    def SensitivityAnalysis(self):
        ...
        net_streamer = self.pyt.NewFileStreamer(ct.c_char_p(self.probpars.baseNET.encode()))
        ...
        for cres in self.probpars.scenario.response:
            vprint(3,self.pyt.verboselvl,"Calculating sensitivity to node --> %s" %(cres))
            ...
            vprint(3,self.pyt.verboselvl,"Deleting sensitivity to --> %s" %(cres))
    ...
    def cross_val_setup(self):
        ...
        for cfold in np.arange(self.probpars.numfolds):
            ...
            for line in outdatLeftOut:
                for cv in line:
                    if np.isnan(cv):
                        ofpLeftOut.write(' {0:24s} '.format('*'))
                    else:
                        ofpLeftOut.write(' {0:24.12e} '.format(cv))
                ofpLeftOut.write('\n')
            ofpLeftOut.close()
